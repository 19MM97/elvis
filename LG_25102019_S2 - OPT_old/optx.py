...
def opt_charging(t, trafoPreload, chargingPoints, assumptions, co2_emission, energy_price):
    ...
    power_base = 100 
    energy_price_base = 30.0
    co2_emission_base = 500.0
    #print(t)
    evs = [s.connectedEV for s in chargingPoints if s.availability == 0]

    data = [ev.__dict__ for ev in evs ]
    data.append({'energy_price': [energy_price[hour] / 1000.0 * 100.0  for hour in hours],
          'breaker_limit': [trafoPreload[3][hour] * 0.8 - trafoPreload[0][hour] for hour in hours],
               'co2_emissions': [co2_emission[hour] * 1000.0 for hour in hours]
               })
...
#    model.breaker_limit = Param(model.time, initialize={hours[t]: data['parameters']['breaker_limit'][t] for t in N})
    ...
    def demand_rule(model, i):
        new_parking_time = min(data[ev_set.index(i)]['parkingTime'], len(hours)/60*tau)
        return sum(getattr(model,'power_' + str(j))[i] for j in model.time) / 60 / model.batterySize[i] + model.soc[i] >= aml.Expr_if(model.socTarget[i] <= new_parking_time *
                                            model.chargingPower[i] / model.batterySize[i] + \
                                            model.soc[i], model.socTarget[i], new_parking_time *
                                            model.chargingPower[i] / model.batterySize[i] +\
                                            model.soc[i])   
    ...
    for w_1, w_2 in zip(assumptions['cost_weights'], assumptions['co2_weights']): 
        def objective_rule(model):
            return sum(w_1 * getattr(model,'power_'+str(j))[i] * model.energy_price[j] / 60.0 * tau
                       for j in model.time for i in model.EVs )/power_base/energy_price_base + \
                   sum(w_2 * getattr(model,'power_'+str(j))[i] * model.co2_emissions[j] / 60.0 * tau
