...
from kpis import get_indicators
import os
import pandas as pd
...
def profile_ev_load(fix_key, assumptions, data):
    ...
    total_load['trafo_preload_kW'] = data.transformer_preload[assumptions['preload']]
...
def main():
    ...
    for car_amount in assumptions['number_of_evs']:  # per week
        ...
        for power in assumptions['power_in_kW']:
            data.power_cp = power
            for lp_amount in assumptions['charging_points_nr']:
                data.amount_cp = lp_amount
                for control in assumptions['control_strategies']:
                    data.control = control
                    for co2_scenario in range(len(assumptions['co2_emissions'])):
                        data.co2_scenario = co2_scenario
                        if control == 'WS':
    ...
    for df, col in zip(load_profiles, df_load_profiles.columns):
        ...
        df_load_profiles[col+'_Trafolast'] = list(df['trafo_loading_kW'][0:1440])
