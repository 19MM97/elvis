...
def opt_charging(t, trafoPreload, chargingPoints, assumptions, co2_emission, energy_price): 
    ...
    def power_bounds_rule(model, i):
        return model.powerMin[i], model.charging_power[i]
        # Parameters
    ...
    for ev in evs:
        connection_time = min(ev.parkingTime, assumptions['Open_hours'][-1]- assumptions['Open_hours'][-1]) \
                          - ev.timeStartXchaging/ 60.0  * tau
        hours = list(range(t, (t // 1440) * 1440 + connection_time * 60.0))
        setattr(model, ev.carID+"time", Set(Set(initialize=hours)))
        data.append({'energy_price'+ev.carID: [energy_price[hour] / 1000.0 for hour in hours],
                     'breaker_limit'+ev.carID: [trafoPreload[3][hour] * 0.8 - trafoPreload[0][hour] for hour in hours],
                     'co2_emissions'+ev.carID: [co2_emission[hour] for hour in hours]
                     })
        for param in grid_param:
            setattr(model, param+ev.carID, Param(getattr(model, ev.carID+'time'),
                                        initialize={hours[hour]: data[-1][param+ev.carID][hour] for hour in
                                                    range(len(hours))}, mutable=True, doc=param+ev.carID))
        for hour in hours:
            setattr(model, ev.carID+'power_'+str(hour),
                    Var(model.EVs, bounds=power_bounds_rule, doc='power given to an EV in kW'))


    #Constraints
    def trafo_limit_rule(model,j, ev):
        return sum(getattr(model, ev.carID+'power_'+str(j))[i] for i in model.EVs for ev in evs) <= getattr(model, "breaker_limit"+ev.carID[j])
    
    model.trafo_limit_rule = Constraint(getattr(model, ev.carID+'time'), rule=trafo_limit_rule)
    
    def demand_rule(model, i):
        connection_time = min(data[ev_set.index(i)]['parkingTime'], len(hours) / 60 * tau) - ((data[ev_set.index(i)]['timeStartXchaging'] - data[ev_set.index(i)]['arrivalTime']) / 60.0 * tau)
        ...
        return sum(
            getattr(model,'power_' + str(j))[i] for j in model.time) / 60 / model.batterySize[i] + model.soc[i] >= aml.Expr_if(
            model.socTarget[i] <= connection_time * model.charging_power[i] / model.batterySize[i] +
            model.soc[i], model.socTarget[i], connection_time * model.charging_power[i] / model.batterySize[i] + model.soc[i])
...
#        print(day_plan)
    ...
    for s in chargingPoints:
        if s.availability == 0: 
            ...
            # print(s.connectedEV.carID, day_ahead_plan[s.connectedEV.carID].sum()/60/s.connectedEV.batterySize, s.connectedEV.soc)
    #
