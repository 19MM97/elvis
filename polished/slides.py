...
class GetSlides: 
    ...
    def getGraphics(self,control,P,n): 
...
#        days = [""]+list(calendar.day_abbr)
        ...
        ax = df_load.groupby(['day_of_week','hour_of_day']).Total_Trafo_LP.mean().unstack().plot(legend=False,kind='box', title= 'load ranges over the day')
        ...
        ax = df_load.groupby(['workingday/weekend', 'hour_of_day']).Total_Trafo_LP.mean().unstack().plot(legend=False,kind='box', title= 'load ranges over the day')
        ax.set_xticks(range(0, 25, 1))
        ax.set_xticklabels(hours,rotation=50)
        ax.set_ylabel('Power/kW')
        ax.set_xlabel('Time')
        ...
        ax = df_load.groupby(['week_of_year', 'day_of_week']).Total_Trafo_LP.mean().unstack().plot(legend=False,kind='box', title= 'load ranges over the days of the week')
        ...
        ax = df_load.groupby(['day_of_week','week_of_year']).Total_Trafo_LP.mean().unstack().plot(legend=False,kind='box', title= 'load ranges the weeks of the year')
    ...
    def get_slides(self, assumptions, indicators, control,P,n):
        self.getGraphics(control,P,n)
        self.get_table(assumptions,'Szenario %s_%s: Annahmen'%(assumptions['Ort'],control))
        self.get_pic_slide('Szenario %s_%s: selected week load profile'%(assumptions['Ort'], control), 'week_LP_%s.png'%control)
        self.get_pic_slide('Szenario %s_%s: year profile grouped by hour of the day'%(assumptions['Ort'], control), 'LP_agg_byhour_%s.png'%control)
        self.get_pic_slide('Szenario %s_%s: year profile grouped by hour of the day_WE/WD'%(assumptions['Ort'], control), 'LP_agg_byhour_we_%s.png'%control)
        self.get_pic_slide('Szenario %s_%s: year profile grouped by day of the week'%(assumptions['Ort'], control), 'LP_agg_byday_%s.png'%control)
        self.get_pic_slide('Szenario %s_%s: year profile grouped byweek of the year'%(assumptions['Ort'], control), 'LP_agg_byweek_%s.png'%control)
...
#        self.get_chart_stack(heading='Szenario %s_%s: Wie viele Autos pro LP? '%(assumptions['Ort'],control), data=servedByEach)