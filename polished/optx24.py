...
for day in range(7):
    ...
    for t in N:
        setattr(model, 'power_'+str(t), Var(model.EVs, bounds=power_bounds_rule, doc='max'))
    ...
    def demand_rule(model, i):
        return sum(getattr(model,'power_'+str(j))[i] for j in model.time)/(tau/60.0)/model.battery_size[i]+model.soc_arr[i] >= 0.8
    ...
    def objective_rule(model):
        return sum(getattr(model,'power_'+str(j))[i] * model.energy_price[j]/(tau/60.0) for j in model.time for i in model.EVs )
    ...
    week_plan = pd.concat([week_plan, day_ahead_plan], axis=0)
summary = {'Number of EVs': len(data['EVs']), 
           'Total charging costs in Euro': model.objective.value()/100.0, 
...
week_plan.to_csv('week_plan_OPT.csv')
